...
nearDist = 10       # how far to smooth near intersections, in meters 
...
newLine_huc = hucPrefix + '_shiftedDitches'

# Stuff created after identifying culverts
culvertBuffers_huc = hucPrefix + '_culvertBuffers'
culvertLines = hucPrefix + '_culvertLines'
newPts = hucPrefix + '_shiftedVertices'
newElevFile = tmpFilesDir + hucPrefix + '_elevProfile_shiftedDitches.txt'

gs.run_command('g.mapset', flags='c', mapset=hucPrefix, project=project)

def shift_countyDitches(ditchPrefix):

    tmpFiles = tmpFilesDir + ditchPrefix + '/'
    ...
    # Shifted lines
    definedLine = ditchPrefix + '_shiftedDitches_notCleaned'
    newLine = ditchPrefix + '_shiftedDitches'
    ...
    if not gdb.map_exists(definedLine, 'vector'):
        ...
        gs.run_command('v.build.polylines', input_=definedLine, output=newLine, \
                        type_='line', cats='first', overwrite=True)

        gs.run_command('v.db.addtable', map=newLine)
        gs.run_command('v.db.addcolumn', map=newLine, columns=['county varchar(250)', 'cat_cty int'])
        gs.run_command('v.db.update', map=newLine, column='county', value=ditchPrefix)
        gs.run_command('v.db.update', map=newLine, column='cat_cty', query_column='cat')

for ditchPrefix in ditchPrefixes:
    shift_countyDitches(ditchPrefix)

ditches_toPatch = [ditchPrefix + '_shiftedDitches' for ditchPrefix in ditchPrefixes]
culverts_toPatch = [ditchPrefix + '_culvertBuffers' for ditchPrefix in ditchPrefixes]
...
if not gdb.map_exists(newPts, 'vector'):
    ...
    gs.run_command('v.overlay', ainput=newLine_huc, atype='line', binput=culvertBuffers_huc, \
                    operator='and', output=culvertLines, overwrite=True)
            
    # Create a burned DEM and one with culvert areas set to null
    filler, demNull = interpSurface.interpSurface(tmpFilesDir, hucPrefix, lineSep, culvertLines, burnWidth, dem, \
                                   demForNull=dem)
        
    # Get elevation profile for shifted ditches
    gs.run_command('v.to.points', input_=newLine_huc, dmax=1, output=newPts)
