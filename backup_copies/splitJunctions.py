...
def split_nodesIntersects(nodeFile, intersectFile, linesLayer):
    ...
    nodesXY['thresh']=1
    intersectsXY['thresh']=0
    breakDf = pd.concat((nodesXY, intersectsXY)).reset_index(drop=True)
    
    haveSplit = pd.DataFrame(columns=['x', 'y'])
    for i in range(len(breakDf)):
        x,y,thresh=breakDf['x'].iloc[i],breakDf['y'].iloc[i],breakDf['thresh'].iloc[i]
        dists = np.sqrt((haveSplit['x']-x)**2 + (haveSplit['y']-y)**2)

        if i==0 or len(np.where(dists < 1)[0]) == 0:
            gs.run_command('v.edit', map_=linesLayer, tool='break', coords=[x,y], threshold=thresh)
...
for tb in [duplicTable1, duplicTable2, intersectTable]:
...
# Delete duplicates (first pass, otherwise connecting lines gets messy)
    ...
    #    gs.run_command('v.edit', map_=vecLines1, tool='connect', threshold=10, ids=i)
    #gs.run_command('v.edit', map_=vecLines1, tool='delete', query='length', threshold=[-1,0,-0.1], type_='line')
